...
export default function ProductDetailPage() {
  ...
  // Kixikila states
  const [kixikilaOpen, setKixikilaOpen] = useState(false);
  const [kixikilaMembersCount, setKixikilaMembersCount] = useState(4);
  const [kixikilaPaidStatus, setKixikilaPaidStatus] = useState<Record<string, boolean>>({
    'Tu': true,
    'Manuel António': true,
    'Alzira Neto': false,
    'Januário Silva': false
  });
  ...
  const simulateKixikilaPayments = () => {
    ...
    setTimeout(() => {
      setKixikilaPaidStatus({
        'Tu': true,
        'Manuel António': true,
        'Alzira Neto': true,
        'Januário Silva': true
      });
      setKixikilaSuccess(true);
      setKixikilaSimulating(false);
    }, 1500);
  };
              ...
              <div className="space-y-3 pt-2">
                ...
                <div className="flex flex-col gap-3">
                  ...
                  <button
                    ...
                    className="w-full bg-amber-500 hover:bg-amber-655 text-slate-950 font-extrabold text-xs py-3.5 rounded-2xl transition-all shadow-md flex items-center justify-center gap-2"
                  >
                  ...
                  <button
                    type="button"
                    onClick={() => setKixikilaOpen(!kixikilaOpen)}
                    className="w-full bg-slate-100 dark:bg-slate-800 hover:bg-slate-200 dark:hover:bg-slate-700 text-slate-900 dark:text-white font-extrabold text-xs py-3.5 rounded-2xl transition-all border border-slate-200 dark:border-slate-700 shadow-sm flex items-center justify-center gap-2"
                  >
                    <span>👥 Iniciar Kixikila (Compra em Grupo)</span>
                  </button>

                  {kixikilaOpen && (
                    <div className="p-4 bg-slate-50 dark:bg-slate-950/40 border border-slate-200 dark:border-slate-800 rounded-2xl space-y-4 animate-fade-in text-xs">
                      <div className="flex items-center justify-between">
                        <span className="font-extrabold text-slate-900 dark:text-white">Kixikila de Compras</span>
                        <span className="bg-amber-500/10 text-amber-600 dark:text-amber-400 font-bold px-2 py-0.5 rounded text-[10px]">
                          Cota: {formatKwanza(finalPrice / kixikilaMembersCount)} / pessoa
                        </span>
                      </div>

                      <div className="space-y-1">
                        <label className="font-bold text-slate-600 dark:text-slate-400">Número de Kambas (Membros):</label>
                        <select
                          value={kixikilaMembersCount}
                          onChange={(e) => setKixikilaMembersCount(Number(e.target.value))}
                          className="w-full bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-xl px-2.5 py-2 text-xs outline-none dark:text-white"
                        >
                          <option value={2}>2 Kambas ({formatKwanza(finalPrice / 2)} cada)</option>
                          <option value={3}>3 Kambas ({formatKwanza(finalPrice / 3)} cada)</option>
                          <option value={4}>4 Kambas ({formatKwanza(finalPrice / 4)} cada)</option>
                        </select>
                      </div>

                      <div className="space-y-2">
                        <span className="font-bold text-slate-650 dark:text-slate-400">Estado de Contribuições:</span>
                        <div className="grid grid-cols-2 gap-2 text-[10px]">
                          {Object.entries(kixikilaPaidStatus).map(([name, paid]) => (
                            <div key={name} className="flex items-center gap-1.5 p-2 rounded-lg bg-white dark:bg-slate-900 border border-slate-100 dark:border-slate-800">
                              <span className={`w-2.5 h-2.5 rounded-full ${paid ? 'bg-emerald-500' : 'bg-amber-500 animate-pulse'}`} />
                      ...
                      {kixikilaSuccess ? (
                        <div className="p-3 bg-emerald-600 text-white font-bold rounded-xl text-center shadow animate-scale-in">
                          🎉 Kixikila Completo! Quotas recebidas em Escrow. Pedido processado com sucesso!
                        </div>
                      ) : (
                        <button
                          type="button"
                          onClick={simulateKixikilaPayments}
                          disabled={kixikilaSimulating}
                          className="w-full bg-amber-500 hover:bg-amber-600 disabled:bg-slate-200 disabled:text-slate-400 text-slate-950 font-extrabold py-2.5 rounded-xl transition-all shadow flex items-center justify-center gap-1.5"
                        >
                          {kixikilaSimulating ? 'A receber depósitos...' : '🧪 Simular Depósitos dos Kambas'}
                        </button>
                      )}
                    </div>
                  )}
